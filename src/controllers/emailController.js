        ...
        input: Buffer.from(`
          ...
          size / 2
            ...
            <text x="50%" y="50%" font-size="${
              size / 2
            }" font-family="Arial" text-anchor="middle" dy=".3em" fill="white">${initial}</text>
          </svg>
        `),
        blend: "over",
      },
...
export const getEmailsWithPagination = async (req, res) => {
  try {
    ...
    const { emailAddress, historyId, messagesTotal, threadsTotal } =
      profile.data;
    ...
    const emailDetailsPromises = messages.map(async (message) => {
      try {
        ...
        const senderHeader =
          email.data.payload?.headers?.find((header) => header.name === "From")
            ?.value || "";
        const messageId =
        ...
        const senderProfileImage = senderEmail
          ? await generateAvatar(senderEmail)
          : null;
        ...
        const replyReceiverHeader =
          email.data.payload?.headers?.find((header) => header.name === "To")
            ?.value || "";
        const replyProfileImage = replyReceiverHeader
          ? await generateAvatar(replyReceiverHeader)
          : null;
...
export const getEmailsTotal = async (req, res) => {
  try {
    ...
    console.error("Error fetching emails:", error);
    res.status(500).json({
      message: "Failed to fetch emails total ",
      error: error.message,
    });
  }
};
...
export const sendBulkEmailController = async (req, res) => {
  try {
    ...
    if (!userId) {
      res
        .status(400)
        .json({ message: "User ID or Google ID is missing or invalid." });
      return;
    }
    ...
    if (!canSendEmail) {
      res
        ...
        .json({
          message:
            "Daily email limit reached. Please try again after 24 hours.",
        });
      return;
    }
...
export const getEmailByMessageId = async (req, res) => {
  try {
    ...
    const senderName = extractName(senderHeader);
    const senderProfileImage = senderEmail
      ? await getGravatarUrl(senderEmail)
      : null;
    const receivedDate = email.data.internalDate
      ? new Date(parseInt(email.data.internalDate)).toISOString()
      : null;
...
const createReplyMessage = (to, fromName, replyText, subject, threadId) => {
  const message = [
